...
class Im2Latex(LightningDataModule):
    ...
    def __init__(
        ...
        # self.data_dirname=Path('/data/zzengae/jwwang/final_project')
        # self.vocab_file = self.data_dirname / "vocab.json"
        self.vocab_file =Path(__file__).resolve().parents[3] / "data" / "vocab1.json"
        formula_file = self.data_dirname / "math_210421/im2latex_formulas.norm.new.lst"
        # formula_file = self.data_dirname / "physic/im2latex_formulas.norm1.lst"
        print(formula_file)
    ...
    def processed_images_dirname(self):
        return self.data_dirname / "formula_images"
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if stage in ("fit", None):
            train_image_names, train_formulas = get_split(
                self.all_formulas,
                self.data_dirname / "math_210421/im2latex_train_filter.lst",
                # self.data_dirname / "physic/im2latex_train_filter1.lst",
            )
            ...
            val_image_names, val_formulas = get_split(
                self.all_formulas,
                self.data_dirname / "math_210421/im2latex_validate_filter.lst",
                # self.data_dirname / "physic/im2latex_validate_filter1.lst",
            )
            # print(val_formulas[0])
        ...
        if stage in ("test", None):
            test_image_names, test_formulas = get_split(
                self.all_formulas,
                self.data_dirname / "math_210421/im2latex_test_filter.lst",
                # self.data_dirname / "physic/im2latex_test_filter1.lst",
            )
    ...
    def collate_fn(self, batch):
        ...
        # print([len(formula) for formula in formulas])
        # print(formulas[0])
        ...
        for i in range(B):
            ...
            # print(formulas[i])
            # print('aaa')
            # res=self.tokenizer.decode(batched_indices[i,:].tolist())
            # print(batched_indices[i,:])
            # print(res)
